...
class SkillPackageTests(unittest.TestCase):
    def test_skill_contains_bootstrap_and_template_contract(self) -> None:
        ...
        self.assertTrue((SKILL / "commands" / "install.md").is_file())
        self.assertTrue((SKILL / "references" / "runtime.md").is_file())
    ...
    def test_startup_command_is_single_valid_live_control_flow(self) -> None:
        ...
        install = (SKILL / "commands" / "install.md").read_text(encoding="utf-8")
        ...
        self.assertIn("plan --source", install)
        self.assertIn("install --source bundled --json", install)
        self.assertNotIn("--plan", install)
        self.assertNotIn("--adopt-or-install", install)
        ...
        self.assertIn("Every request in `requests.jsonl`", startup)
        self.assertNotIn("正在等待微信指令", startup)
        self.assertIn("--dangerously-bypass-approvals-and-sandbox", install)
